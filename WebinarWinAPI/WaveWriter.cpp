...
BOOL WaveWriter::Initialize(const WCHAR* wszFile, const BOOL bExtensibleFormat)
{
	BOOL bRet = FALSE;
...
BOOL WaveWriter::WriteWaveData(const BYTE* pData, const DWORD dwLength)
{
	BOOL bRet = FALSE;
	DWORD dwWritten;
...
BOOL WaveWriter::FinalizeHeader(WAVEFORMATEX* pwfx, const UINT32 uiFileLength, const BOOL bExtensibleFormat)
{
	BOOL bRet = FALSE;
	DWORD dwMove;
	DWORD dwWritten;
...
BOOL WaveWriter::SetWaveHeaderExt(WAVEFORMATEX* pwfx, const UINT32 uiDataLen, BYTE* head)
{
	if(uiDataLen == 0)
		return FALSE;
...
BOOL WaveWriter::SetWaveHeader(const WAVEFORMATEX* pwfx, const UINT32 uiDataLen, BYTE* head)
{
	if(uiDataLen == 0)
		return FALSE;
